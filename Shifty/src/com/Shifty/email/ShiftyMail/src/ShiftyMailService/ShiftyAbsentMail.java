    ...
    private static Message prepareMessage(Session session, String shiftyEmailAccount, String recepient) {
        try{
            ...
            message.setText("\nThis is a Shifty Email \n\n"
                    + "You have recieved this message becasue you have not come to WORK! \n"
                    + "You better have a very good explanation because you are in deep trouble. \n\n"
                    + "Thank you for your attention.\n\n"
                    + "SEE YOU AT WORK!"); // actual message to be sent
            
            return message;
        } catch (MessagingException ex) {   //exeption for email to get into email
            Logger.getLogger(ShiftyAbsentMail.class.getName()).log(Level.SEVERE, null, ex);
        }
        return null;  
    }

    private ShiftyAbsentMail() {
    }
